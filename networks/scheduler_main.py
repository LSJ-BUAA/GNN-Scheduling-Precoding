...
class Scheduler(nn.Module):
    # the scheduler module in SGNN (Sequential GNN, including K' 3D-GNNs)
    def __init__(self):
    ...
    def forward(self, A, check_repeated=False):
        ...
        sumb = torch.zeros([BATCH_SIZE, 1, M, K]).to(device) # scheduled result, to be passed sub-scheduler (3D-GNN) by sub-scheduler (3D-GNN)
        b = torch.zeros([BATCH_SIZE, NRF, M, K]).to(device) # this is tensor B in the paper
...
class NonSeqScheduler(nn.Module):
    # the scheduler module in NGNN (non-Sequential GNN), including one 3D-GNN
    def __init__(self):
...
if __name__ == '__main__':
    ...
    # hyper parameters
    BATCH_SIZE = 50 # or 100 or 500, could be large if GPU memory allowed
    ...
    tau0 = 0.5
    tauinf = 0.1
    # tau0 = 0.05 # or 0.1 if NRF is large, or if repeat_number is large
    # tauinf = 0.02

    MAX_EPOCH = 300
    is_attention = False # always False
    test_equal = False # always False
    is_sequential = True #True for SGNN, False for NGNN

    device = torch.device("cuda:0")
    # device = torch.device("cpu")

    epoch_train_precoder = 10 # or 20 if NRF is large
    w1 = 8 # or 6 when NRF is small and K is large, or 12 when NRF is large
    w2 = 4 # or 2, in fact strength is more important, this is negligible
    ...
    if not is_model_feature: #it is ``SGNN w/o F''
        # should also replace self.gnnM.append(GNN3D(input_dim=2+2+1, hidden_dim=layerM, output_dim=1)) by self.gnnM.append(GNN3D(input_dim=2+1, hidden_dim=layerM, output_dim=1)) and do not use artificial_feature
        w1 = 0
        w2 = 0
    ...
    print('Sequential',is_sequential,'BatchSize',BATCH_SIZE,'ini_LEARNING_RATE',LEARNING_RATE,LR2,'layerM',layerM,'atten',is_attention, 'epoch_train_precoder',epoch_train_precoder, 'feature_weight',w1,w2)
    ...
    precoder.to(device)
    precoder.load_state_dict(torch.load('model_M'+str(M)+'_NRF'+str(NRF)+'_K'+str(K)+'_Kp'+str(NRF)+'_NT'+str(NT)+'_NR'+str(NR)+'_P'+str(power_dBm)+'_E'+str(90)+'_128_6_200k.ckp')['model_state_dict'])
    ...
    # model.load_state_dict(torch.load('Smodel_M'+str(M)+'_NRF'+str(NRF)+'_K'+str(K)+'_Kp'+str(NRF)+'_NT'+str(NT)+'_NR'+str(NR)+'_P'+str(power_dBm)+'_E'+str(10)+'_32_2_200k.ckp')['model_state_dict'])
    # for p in model.parameters():
    #     p.requires_grad = True
    # optimizer.load_state_dict(torch.load('Smodel_M'+str(M)+'_NRF'+str(NRF)+'_K'+str(K)+'_Kp'+str(NRF)+'_NT'+str(NT)+'_NR'+str(NR)+'_P'+str(power_dBm)+'_E'+str(10)+'_32_2_200k.ckp')['optimizer_state_dict'])
    # scheduler.load_state_dict(torch.load('Smodel_M'+str(M)+'_NRF'+str(NRF)+'_K'+str(K)+'_Kp'+str(NRF)+'_NT'+str(NT)+'_NR'+str(NR)+'_P'+str(power_dBm)+'_E'+str(10)+'_32_2_200k.ckp')['scheduler_state_dict'])
    # precoder.load_state_dict(torch.load('Smodel_M' + str(M) + '_NRF' + str(NRF) + '_K' + str(K) + '_Kp' + str(NRF) + '_NT' + str(NT) + '_NR' + str(NR) + '_P' + str(power_dBm) +'_E'+str(10)+ '_32_2_200k.ckp')['Pmodel'])
    # for p in precoder.parameters():
    #     p.requires_grad = True
    # optimizer1.load_state_dict(torch.load('Smodel_M' + str(M) + '_NRF' + str(NRF) + '_K' + str(K) + '_Kp' + str(NRF) + '_NT' + str(NT) + '_NR' + str(NR) + '_P' + str(power_dBm) + '_E' + str(10) + '_32_2_200k.ckp')['Pop'])
    # scheduler1.load_state_dict(torch.load('Smodel_M' + str(M) + '_NRF' + str(NRF) + '_K' + str(K) + '_Kp' + str(NRF) + '_NT' + str(NT) + '_NR' + str(NR) + '_P' + str(power_dBm) + '_E' + str(10) + '_32_2_200k.ckp')['Psc'])
            ...
            with torch.no_grad():
                ...
                for b in range(int(test_number / BATCH_SIZE)):
                    ...
                    for m in range(len(Mtest)):
                        ...
                        h_pred, x_pred, y_pred, z_pred = model(batch_x)
                        loss = -cal_sumrate(H=h_pred, WRF=z_pred, WBB=y_pred, VRFH=x_pred, SNR_dB=SNRtest[m], device=device)
