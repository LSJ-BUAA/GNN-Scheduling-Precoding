...
# trains scheduler and precoder for generalizing the number of UE-ANs (NR)
...
if __name__ == '__main__':
    ...
    NTread = NT2
    dataTest = []
    for n in range(len(NRtest)):
        NRread = NRtest[n]
        dict_data = h5py.File('../data/ChannelUMaNLOSRB' + str(RBread) + 'NT' + str(NTread) + 'NR' + str(NRread) + 'K' + str(Kread) + '.mat')
        data = dict_data['CHRB']  # number,NR,NT,M2

        data = data[0:test_number * Kin2, :, :, 0:RBread].reshape([test_number, Kin2, NRread, NTread, RBread])
        data = data.transpose(0, 4, 1, 2, 3)
    ...
    # make data set
    Xtrain = torch.from_numpy(np.float32(dataTrain[0:train_number,:,0:M]))*math.sqrt(NR)/math.sqrt(2) # to counteract the RX normalization during channel generation, with NR=2 as the baseline
    Xtest = []
    for n in range(len(NRtest)):
        Xtest.append(torch.from_numpy(np.float32(dataTest[n][0:test_number,:,0:M2]))*math.sqrt(NRtest[n])/math.sqrt(2))
    ...
    print('Sequential',is_sequential,'BatchSize',BATCH_SIZE,'ini_LEARNING_RATE',LEARNING_RATE,LR2,'layerM',layerM,'atten',is_attention, 'epoch_train_precoder',epoch_train_precoder, 'feature_weight',w1,w2,'NRtest',NRtest)
    ...
    precoder.to(device)
    precoder.load_state_dict(torch.load('model_M'+str(M)+'_NRF'+str(NRF)+'_K'+str(K)+'_Kp'+str(NRF)+'_NT'+str(NT)+'_NR'+str(NR)+'_P'+str(power_dBm)+'_E'+str(90)+'_128_8_200k.ckp')['model_state_dict'])
